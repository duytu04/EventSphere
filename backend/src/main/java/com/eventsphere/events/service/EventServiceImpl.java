...
// src/main/java/com/eventsphere/events/service/EventServiceImpl.java
package com.eventsphere.events.service;
...
public class EventServiceImpl implements EventService {
  ...
  public Event create(EventCreateRequest req, Long organizerId){
    ...
    var e = Event.builder()
      ...
      .startTime(req.startTime())
      .endTime(req.endTime())
      .totalSeats(req.capacity() != null ? req.capacity() : 0)
      .seatsAvail(req.capacity() != null ? req.capacity() : 0)
      .status(ApprovalStatus.PENDING_APPROVAL.name())
      .organizerId(organizerId)
      .build();
    return repo.save(e);
  ...
  public Event update(Long id, EventUpdateRequest req){
    ...
    // Giữ nguyên update theo DTO hiện tại của bạn (title/venue/totalSeats ...)
    e.setTitle(req.title().trim());
    e.setDescription(req.description());
    e.setCategory(req.category());
    e.setVenue(req.venue());
    ...
    e.setTotalSeats(req.totalSeats());
    e.setSeatsAvail(Math.max(0, req.totalSeats() - consumed)); // seatsAvail = totalSeats - consumed
    return e;
  }
